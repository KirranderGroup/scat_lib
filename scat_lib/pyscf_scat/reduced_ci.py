"""  # moved into scat_lib.pyscf_scat
Reduced CI Module
...
try:
    ...
    # Mock the classes for documentation builds
    class mcscf:
        class mc1step:
            class CASSCF:
                pass
...
try:
    ...
    # For documentation builds, create dummy functions
    class scat_calc:
        @staticmethod
        ...
        def run_scattering_csf(*args, **kwargs):
            pass
        @staticmethod
        def run_scattering_pyscf(*args, **kwargs):
            pass
    
    class ci_to_2rdm:
        @staticmethod
        def write_ci_file(*args, **kwargs):
            pass
        @staticmethod
        def read_ci_file(*args, **kwargs):
            pass
        @staticmethod
        def update_ci_coeffs(*args, **kwargs):
            pass
        @staticmethod
        def calc_energy(*args, **kwargs):
            pass
    
    class fit_utils:
        @staticmethod
        def generate_comparison_plot(*args, **kwargs):
            pass
...
class ReducedCASSCF(mcscf.mc1step.CASSCF):
    ...
    def write_ci(self, file_name, tol = None):
        ...
        if tol is not None:
            with open(f'{file_name}.ci', 'w') as f:
    ...
    def write_csf(self, file_name):
        with open(f'{file_name}.csf', 'w') as f:
    ...
    def update_ci(self, value):
        norm = (value**2).sum()
        _ = ci_to_2rdm.update_ci_coeffs(self.alpha_dets, self.beta_dets, value, self, update = True)
        _ = self.calc_etot()
