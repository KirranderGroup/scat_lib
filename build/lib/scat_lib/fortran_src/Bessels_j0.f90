...
implicit none
    ...
    subroutine bessels0rr(h_sum, order, mu, H, h_saved)
        ...
        integer :: order2
        integer(kind = ikind) :: i, beta, ra, maxord, ind,qsize
        ...
        !do qsize=1,size(mu)
        !       if (abs(pmu(i))>1e-10) then
        !        h_sum(qsize) = sinpmu(qsize)/pmu(qsize) * h_saved(1)
       !        else
       !         h_sum(qsize)= h_saved(1)
       !        end if 
       ! enddo
        ...
        elseif(order>=4) then
           ...
           ! allbessels(1,1)=1.d0
           ! allbessels(2:order+1,1)=0.d0
           ! h_1 = (sinpmu / Pmu**2 - cospmu / Pmu)
            
           ! if (sum(abs((h_1-allbessels(2,1:size(mu)))))>1e-4) then 
           !    print*,'increase the maximum bessel', sum(abs((h_1-allbessels(2,1:size(mu)))))
           !    do i=1,size(mu)
           !    print*,h_1(i),allbessels(2,i),pmu(i),h_1(i)-allbessels(2,i)
           !    enddo
           !    stop
           ! end if 
           !if (order>6) then
            !    print*,order
            !    print*,pmu(450),allbessels(order-2,450)
            !    PRINT*,pmu(450),allbessels(order-1,450)
            !    PRINT*,pmu(450),allbessels(order,450)
            !    print*,pmu(450),allbessels(order+1,450)
           !endif 
            do beta = 1, order
