#!/usr/bin/env python
import sys
sys.path.append('/u/ajmk/sann8252/PyXSCAT_Patrick/src')
...
qmax = 250.
npoints = 1000
cutoffcentre = 1E-2  # suggested: cutoffcentre = 0.01;
...
readrdm = True
file_read_onerdm = '1rdm_pyscf.txt'
file_read_twordm = '2rdm_pyscf.txt'

if not jeremyR and not hf and not readrdm:
...
if molcas:
    ...
    Nmo_max = 100
    gtos, atoms = pymldreader.read_orbitals(mldfile, N=Nmo_max, decontract=True)
...
with open('options.dat', 'w') as f:
    ...
    f.write(str(molcas) + '\n')
    f.write(str(bagel) + '\n')
    if molpro and not readrdm and not hf:
                    ...
                    for j in range(np.size(confs2)):
                        if confs2[j] != 0:
                            if np.mod(j, 2) == 0:
                                ...
                                beta = beta + 2 ** ((j - 1) / 2)
                    if Type == 9:
                        m.write(str(i) + ' ' + str(civs[i, state1 - 1]) + ' ' + str(civs[i, state2 - 1]) + ' ' + str(civs[i, state3 - 1]) + ' ' 
                            + str(int(alpha)) + ' ' + str(int(beta)) + '\n')
                    else:
                        m.write(str(i) + ' ' + str(civs[i, state1 - 1]) + ' ' + str(civs[i, state2 - 1]) + ' ' 
                            + str(int(alpha)) + ' ' + str(int(beta)) + '\n')
        f.write(str(bitwise) + '\n')
