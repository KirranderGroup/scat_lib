...
contains
    ...
    subroutine total_scattering_j0_groups_fast(q, l, m, n, ngto, ng, nq, maxl, typec, state1, &
            ...
            cutoffmd, cutoffz, cutoffcentre,contrvec,readmat, result2)
        implicit none
        ...
        integer(kind = ikind), intent(in) :: ngto, ng, nq, maxl, typec, state1, state2, ncontr, readmat
...
999 continue
    ...
    subroutine tot_integration(ncap,ncontr, px, py, pz, l, m, n, p0matrix, dx, dy, dz,&
        ...
        integer(kind=int64)::newcap
        real(kind = SELECTED_REAL_KIND(8)), dimension(:, :, :, :), allocatable :: Zbig
        ...
        !            do r=k+1,ncap
         !               bigvec(1,count2)=i
          !              bigvec(2,count2)=j
           ...
           ! enddo
       ! enddo
        print*,'allocated and filled bigvec'
        ...
        close(40)
        print*,'starting loop'
        print*,'The number of contractions is ', ncontr
        ...
        !$OMP& shared( cutoffz,zbig,posits,cutoffmd,group_count,group_start) REDUCTION(+:tsi), &
